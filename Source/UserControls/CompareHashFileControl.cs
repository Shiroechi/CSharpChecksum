...
namespace CSharpChecksum.UserControls
{
	public partial class CompareHashFileControl : UserControl
	{
		...
		~CompareHashFileControl()
		{
			...
			this.BrowseTextFileButton.Dispose();
		}
		...
		private void BrowseTextFileButton_Click(object sender, EventArgs e)
		{
			using (OpenFileDialog openFileDialog = new OpenFileDialog())
			{
				openFileDialog.InitialDirectory = "D:\\";
				...
				if (FileLocationTextBox.Text.Trim() != "")
				{
					openFileDialog.InitialDirectory = GetFileDirectory(FileLocationTextBox.Text);
				}

				if (openFileDialog.ShowDialog() == DialogResult.OK)
				{
					this.HashValueTextBox.Text = File.ReadAllText(openFileDialog.FileName.Trim());
				}
			}
		}
		...
		private void Hahsing_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			...
			else if (this.HashValueTextBox.Text == e.Result.ToString())
			{
				MessageBox.Show(this, "Same", "Attention");
			}
			else
			{
				MessageBox.Show(this, "Different", "Attention");
			}
